...
//ASCII (American Standard Code for Information Interchange).For example, uppercase A = 65, asterisk (*) = 42, and lowercase k = 107.
//
...
{
	unsigned char ch;
	fstream file("p059_cipher.txt", ios::in);
	int no = 0;
//	char key[] = {'g', 'o', 'd'};
	int i = 0, sum = 0;
	...
	while(!file.eof())
	{
		...
		{
//			cout<<no<<",";
			encoded.push_back(no);
			no = 0;
			continue;
		}
		no = no * 10 + (int(ch) - 48);
	}
	encoded.push_back(94);
	for(unsigned char i = 'a'; i <= 'z'; i++)
		for(unsigned char j = 'a'; j <= 'z'; j++)
			for(unsigned char k = 'a'; k <= 'z'; k++){
				...
				{
					sum += (encoded[p] ^ key[p % 3]);
					decoded.push_back(encoded[p] ^ key[p % 3]);
				}
				bool valid = true;
				for(auto x : decoded)
				{
...
//					valid |= (x == '/' || x == '&' || x == '^' || x == '\"' || x == '#');
//					valid |= (x == '')
					valid = ((x >= 32  && x <= 59) || (x >= 65 && x <= 93) || (x >= 97 && x <= 125));
					if(!valid)
						break;
				}
				sum = 0;
